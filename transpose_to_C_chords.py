...
def prepare_chord_label_BCMCL(output):
    for file_name in os.listdir(output):
        if os.path.isfile(os.path.join(output, 'transposed_') + 'KBCKE' + file_name) or os.path.isfile(os.path.join(output, 'transposed_') + 'KBC_oriKE' + file_name) \
                or os.path.isfile(os.path.join(output, 'transposed_') + 'KBa_oriKE' + file_name) or os.path.isfile(os.path.join(output, 'transposed_') + 'KBaKE' + file_name):
            continue
        if file_name[-3:] == 'txt' and file_name.find('KB') == -1 and file_name.find('transposed') == -1:
            print(file_name)
            s = converter.parse(os.path.join(output, file_name[:-3] + 'xml'))
            transpose_chord_label(s, output, file_name)


def transpose_chord_label(s, output, file_name):
    ...
    for key_transpose in range(12):
        if k.mode == 'minor':
            transposed_interval = interval.Interval(k.tonic,
                                                    pitch.Pitch(c2[(displacement - key_transpose - 3) % len(c2)]))
        ...
        p = re.compile(r'[#-]+')
        for line_total in f.readlines(): # can possibly have two or three labels
            line_list = line_total.split(' ')  # this saves all the answers in list
            line_result = []  # this saves the transposed results
            for line in line_list:
                ...
                if id_id != []:  # has flat or sharp
                    ...
                    line_result.append(transposed_result)
                    ...
                    # print(transposed_result, end='', file=fnew)
                    # print('original: ', line, 'transposed: ', transposed_result)
            print(" ".join(map(str, line_result)), end='', file=fnew)
