...
def get_predict_file_name():
    ...
    for fn in os.listdir(cwd):
        if fn[-3:] == 'mid':
            if (os.path.isfile('.\\useful_chord_symbols\\train\\translated_transposed_' + fn[0:3] + '.pop''')):
                continue
            elif (os.path.isfile('.\\useful_chord_symbols\\train\\translated_transposed_' + fn[0:3] + '.pop.not''')):
                continue
            elif (os.path.isfile('.\\useful_chord_symbols\\valid\\translated_transposed_' + fn[0:3] + '.pop''')):
                continue
            elif (os.path.isfile('.\\useful_chord_symbols\\valid\\translated_transposed_' + fn[0:3] + '.pop.not''')):
                continue
            elif (os.path.isfile('.\\useful_chord_symbols\\test\\translated_transposed_' + fn[0:3] + '.pop''')):
                continue
            elif (os.path.isfile('.\\useful_chord_symbols\\test\\translated_transposed_' + fn[0:3] + '.pop.not''')):
                continue
            else:
                  # skip the file which does not have chord labels
...
def train_and_predict(layer,nodes,windowsize,portion):
    ...
    train_xx = np.loadtxt('traintraintrain_x_windowing_1.txt')
    train_yy = np.loadtxt('traintraintrain_y_windowing_1.txt')
    valid_xx = np.loadtxt('validvalidvalid_x_windowing_1.txt')
    valid_yy = np.loadtxt('validvalidvalid_y_windowing_1.txt')
    test_xx = np.loadtxt('testtesttest_x_windowing_1.txt')
    test_yy = np.loadtxt('testtesttest_y_windowing_1.txt')
    ...
    test_xx = np.loadtxt('trainvalidtest_x_windowing_1_230.txt')
    predict_y = model.predict(test_xx, verbose=0)
